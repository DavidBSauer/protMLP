...
threshes.add_argument("-th", "--th_threshold",action='store', type=float, help="Z-score threshold of the tophat fits. Used to exclude columns in the MSA. Default is "+str(th_threshold)+'.',dest='th_threshold',default=th_threshold)
threshes.add_argument("-pb", "--pb_threshold",action='store', type=float, help="Z-score threshold of the point-biserial correlation coefficients. Used to exclude columns in the MSA. Default is "+str(bs_threshold)+'.',dest='bs_threshold',default=bs_threshold)
...
logger.info('The mean training OGT: '+str(np.mean(onehot_data['train']['target'])))
...
plt.legend()
plt.xlabel('OGT values')
plt.ylabel('Number of Sequences')
plt.savefig('./results/OGT_histogram.png')
plt.cla()
plt.clf()
plt.close()
...
plt.xlabel('OGT values')
plt.ylabel('Number of Sequences')
plt.savefig('./results/OGT_histogram_log.png')
plt.cla()
plt.clf()
plt.close()
...
if balanced:
	...
	plt.xlabel('OGT values')
	plt.ylabel('Number of Sequences')
	plt.savefig('./results/OGT_histogram_balanced.png')
	plt.cla()
	plt.clf()
	plt.close()
	...
	plt.xlabel('OGT values')
	plt.ylabel('Number of Sequences')
	plt.savefig('./results/OGT_histogram_balanced_log.png')
	plt.cla()
	plt.clf()
	plt.close()

	logger.info('Number of training sequences after balancing: '+str(onehot_data['train']['target'].shape[0]))
	logger.info('The mean training OGT after balancing: '+str(np.mean(onehot_data['train']['target'])))
