...
def setup(all_data,my_unit,parallel):
...
def final_eval(input):
	...
	return {'NN':NN,'MSE':MSE,'RMSE':RMSE,'r':r,'data_param':data_param,'residuals':test_resid_dict,'folder':folder}

def knn(inputs):
	"""Train a k-Nearest-Neighbors regression"""
	(trials,threads,verbose) = inputs
	folder = './results/kNN'
	os.mkdir(folder)
	...
	def knn_neighbors(inputs):
		...
		return {'mse':mse,'rmse':rmse,'k':n,'r':r}
	
	n_range = [(n,train_data.values,train_target.values,valid_data.values,valid_target.values) for n in range(1,4,1)] #range(1,int(n_train_seq*0.01),1)]
	if verbose:
	...
	return {'MSE':MSE,'RMSE':RMSE,'r':r,'residuals':test_resid_dict,'folder':folder,'results':results}	
